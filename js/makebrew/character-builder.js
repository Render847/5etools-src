...
export class CharacterBuilder extends BuilderBase {
	...
	setStateFromLoaded(state) {
		...
		// auto-set proficiency toggles (1 = proficient; 2 = expert, 3 = halfproficient)
		if (!state.m.profSave) {
			state.m.profSave = {};
			if (state.s.save) {
		...
		if (!state.m.profSkill) {
			...
			if (state.s.skill) {
				...
				Object.entries(state.s.skill).forEach(([prop, val]) => {
					...
					const expectedHalfprof = abilMod + Math.floor(pb / 2);
					if (Number(val) === Number(expectedHalfprof)) state.m.profSkill[prop] = 3;
				});
			}
		}
		...
		if (!state.m.autoCalc) {
			...
			if (state.s.hp.formula && state.s.hp.average != null) {
				...
				const parts = CharacterBuilder.__getHpInput__getFormulaParts(state.s.hp.formula);
				if (parts) {
	...
	__getAlignmentInput(cb) {
		...
		if ((this._state.alignment && this._state.alignment.some(it => it && (it.special != null || it.alignment !== undefined))) || !~CharacterBuilder.__getAlignmentInput__getAlignmentIx(this._state.alignment)) {
			this._state.alignment.forEach(alignment => CharacterBuilder.__getAlignmentInput__getAlignmentRow(doUpdateState, alignmentRows, alignment).wrp.appendTo(wrpRows));
		} else {
			CharacterBuilder.__getAlignmentInput__getAlignmentRow(doUpdateState, alignmentRows, this._state.alignment).wrp.appendTo(wrpRows);
		}
		...
		ee`<button class="ve-btn ve-btn-xs ve-btn-default">Add Alignment</button>`
			.appendTo(wrpBtnAdd)
			.onn("click", () => {
				CharacterBuilder.__getAlignmentInput__getAlignmentRow(doUpdateState, alignmentRows).wrp.appendTo(wrpRows);
				doUpdateState();
			});
	...
	static __getAlignmentInput__getAlignmentRow(doUpdateState, alignmentRows, alignment) {
		...
		const getAlignment = () => {
			switch (selMode.val()) {
				case "0": {
					return [...CharacterBuilder._ALIGNMENTS[selAlign.val()]];
				}
				case "1": {
					const out = { alignment: [...CharacterBuilder._ALIGNMENTS[selAlign.val()]] };
		...
		const selAlign = ee`<select class="form-control input-xs mb-2">${CharacterBuilder._ALIGNMENTS.map((it, i) => it ? `<option value="${i}">${Parser.alignmentListToFull(it).toTitleCase()}</option>` : `<option disabled>\u2014</option>`).join("")}</select>`
			.onn("change", () => doUpdateState());
		const stageSingle = ee`<div>${selAlign}</div>`.toggleVe(initialMode === "0" || initialMode === "1");
		initialMode === "0" && alignment && selAlign.val(`${CharacterBuilder.__getAlignmentInput__getAlignmentIx(alignment.alignment || alignment)}`);
		initialMode === "1" && alignment && selAlign.val(`${CharacterBuilder.__getAlignmentInput__getAlignmentIx(alignment.alignment)}`);
	...
	static __getAlignmentInput__getAlignmentIx(alignment) {
		return CharacterBuilder._ALIGNMENTS.findIndex(it => CollectionUtil.setEq(new Set(it), new Set(alignment)));
	}
	...
	__getAcInput(cb) {
		...
		const wrpRows = ee`<div></div>`.appendTo(rowInner);
		this._state.ac.forEach(ac => CharacterBuilder.__getAcInput__getAcRow(ac, acRows, doUpdateState).wrp.appendTo(wrpRows));

		const wrpBtnAdd = ee`<div></div>`.appendTo(rowInner);
		ee`<button class="ve-btn ve-btn-xs ve-btn-default">Add Armor Class Source</button>`
			.appendTo(wrpBtnAdd)
			.onn("click", () => {
				CharacterBuilder.__getAcInput__getAcRow(null, acRows, doUpdateState).wrp.appendTo(wrpRows);
				doUpdateState();
			});
	...
	static __getAcInput__getAcRow(ac, acRows, doUpdateState) {
		...
		const selMode = ee`<select class="form-control input-xs mkbru_mon__ac-split">
			...
			switch (selMode.val()) {
				...
				case "1": {
					...
					iptAc.showVe();
					iptSpecial.hideVe();
					if (!fromRows.length) CharacterBuilder.__getAcInput__getFromRow(null, fromRows, doUpdateState).wrpFrom.appendTo(wrpFromRows);
					doUpdateState();
					break;
				}
				case "2": {
		...
		if (ac && ac.from) ac.from.forEach(f => CharacterBuilder.__getAcInput__getFromRow(f, fromRows, doUpdateState).wrpFrom.appendTo(wrpFromRows));

		const btnAddFrom = ee`<button class="ve-btn ve-btn-xs ve-btn-default mb-2">Add Another Feature/Item</button>`
			.onn("click", () => {
				CharacterBuilder.__getAcInput__getFromRow(null, fromRows, doUpdateState).wrpFrom.appendTo(wrpFromRows);
				doUpdateState();
			});
	...
	static __getAcInput__getFromRow(from, fromRows, doUpdateState) {
		...
		const menu = ContextUtil.getMenu(Object.keys(CharacterBuilder._AC_COMMON).map(k => {
			return new ContextUtil.Action(
				k,
				() => {
					iptFrom.val(CharacterBuilder._AC_COMMON[k]);
					doUpdateState();
				},
			);
		}));
	...
	__getHpInput(cb) {
		...
		const initialMode = (() => {
			if (this._state.hp.special != null) return "2";
				...
				const parts = CharacterBuilder.__getHpInput__getFormulaParts(this._state.hp.formula);
		...
		if (initialMode === "0") {
			const formulaParts = CharacterBuilder.__getHpInput__getFormulaParts(this._state.hp.formula);
	...
	__getSkillInput(cb) {
		...
		const getRow = (name, prop) => {
			...
			const iptVal = ee`<input class="form-control form-control--minimal input-xs mr-2 ve-text-center">`
				.onn("change", () => {
					if (this._meta.profSkill[prop]) {
						btnHalfprof.removeClass("active");
						btnProf.removeClass("active");
			...
			const _handleButtonPress = (profMult) => {
				if (profMult === 2) {
				...
				} else if (profMult === 0.5) {
					if (this._meta.profSkill[prop] === 3) {
						delete this._meta.profSkill[prop];
						iptVal.val("");
						this.__getSaveSkillInput__handleValChange(cb, "skill", iptVal, prop);
					} else {
						this._meta.profSkill[prop] = 3;
						hook();
					}
					btnHalfprof.toggleClass("active", this._meta.profSkill[prop] === 3);
					btnProf.removeClass("active");
					btnExpert.removeClass("active");
				} else {
			...
			const btnHalfprof = ee`<button class="ve-btn ve-btn-xs ve-btn-default ml-2" title="Is Half Proficient">Half</button>`
				.onn("click", () => _handleButtonPress(0.5));
			if (this._meta.profSkill[prop] === 3) btnHalfprof.addClass("active");

			const btnProf = ee`<button class="ve-btn ve-btn-xs ve-btn-default" title="Is Proficient">Prof.</button>`
				.onn("click", () => _handleButtonPress(1));
			...
			const btnExpert = ee`<button class="ve-btn ve-btn-xs ve-btn-default ml-2" title="Has Expertise">Expert.</button>`
				.onn("click", () => _handleButtonPress(2));
			...
			const hook = () => {
				if (this._meta.profSkill[prop] === 1) _setFromAbility(1);
				else if (this._meta.profSkill[prop] === 2) _setFromAbility(2);
				else if (this._meta.profSkill[prop] === 3) _setFromAbility(0.5);
			};
			...
			${iptVal}${btnProf}${btnExpert}${btnHalfprof}
			</div>`;
		};
	...
	__getDefensesInput(cb, rowName, shortName, prop) {
		...
		const doAddGroup = data => {
			const group = CharacterBuilder.__getDefensesInput__getNodeGroup(shortName, prop, groups, doUpdateState, 0, data);
			groups.push(group);
	...
	static __getDefensesInput__getNodeGroup(shortName, prop, groups, doUpdateState, depth, initial) {
				...
				() => {
					const child = (() => {
						...
						if (i < optionsList.length) {
							...
							return CharacterBuilder.__getDefensesInput__getNodeItem(shortName, children, doUpdateState, optionsList[i]);
						} else { // "Special"
							if (alreadyExists("special")) return null;
							return CharacterBuilder.__getDefensesInput__getNodeItem(shortName, children, doUpdateState, "special");
						}
					})();
		...
		const btnAddChildGroup = ee`<button class="ve-btn ve-btn-xs ve-btn-default mr-2">Add Child Group</button>`
			.onn("click", () => addChild(CharacterBuilder.__getDefensesInput__getNodeGroup(shortName, prop, children, doUpdateState, depth + 1)));
		...
		if (initial) {
			...
			initial[prop].forEach(dmgType => {
				if (typeof dmgType === "string") addChild(CharacterBuilder.__getDefensesInput__getNodeItem(shortName, children, doUpdateState, dmgType), false);
				else if (dmgType.special != null) addChild(CharacterBuilder.__getDefensesInput__getNodeItem(shortName, children, doUpdateState, "special", dmgType.special), false);
				else addChild(CharacterBuilder.__getDefensesInput__getNodeGroup(shortName, prop, children, doUpdateState, depth + 1, dmgType), false);
			});
		}

		const out = { getState, ele, type: "group", order: CharacterBuilder._rowSortOrder++ };
		return out;
	}
	...
	static __getDefensesInput__getNodeItem(shortName, children, doUpdateState, type, value) {
		...
		const out = { ele, getState, type, order: CharacterBuilder._rowSortOrder++ };
		return out;
	}
	...
	__getLanguageInput(cb) {
		...
		const availLanguages = Object.entries(Parser.MON_LANGUAGE_TAG_TO_FULL).filter(([k]) => !CharacterBuilder._LANGUAGE_BLOCKLIST.has(k))
	...
	__getSpellcastingInput__getSpellGenericRow(spellRows, doUpdateState, meta, data) {
		...
		const btnAdd = ee`<button class="ve-btn ve-btn-xxs ve-btn-default mr-2" title="Add Spell"><span class="glyphicon glyphicon-plus"></span></button>`
			.onn("click", async () => {
				...
				const spell = await SearchWidget.pGetUserSpellSearch(options);//here have new menu launch search/filter/pinned list
				if (spell) {
					addItem(spell.tag);
					doUpdateState();
				}
		...
		const addItem = spell => {
			const item = CharacterBuilder.__getSpellcastingInput__getSpellGenericRow__getRowItem(rowItems, doUpdateState, spell);
			rowItems.push(item);
	...
	static __getSpellcastingInput__getSpellGenericRow__getRowItem(rowItems, doUpdateState, spellEntry) {
		...
		const out = { ele, getState, order: CharacterBuilder._rowSortOrder++ };
		return out;
	}
	...
	__getEnvironmentInput(cb) {
		...
		if (additionalEnvs.length) {
			additionalEnvs.forEach(it => {
				CharacterBuilder.__getEnvironmentInput__getCustomRow(doUpdateState, inputs, it).ele.appendTo(wrpIpts);
			});
		}

		const btnAddCustom = ee`<button class="ve-btn ve-btn-default ve-btn-xs mt-2">Add Custom Environment</button>`
			.onn("click", () => {
				CharacterBuilder.__getEnvironmentInput__getCustomRow(doUpdateState, inputs).ele.appendTo(wrpIpts);
			});
...
CharacterBuilder._AC_COMMON = {
	"Unarmored Defense": "unarmored defense",
	"Natural Armor": "natural armor",
};
CharacterBuilder._LANGUAGE_BLOCKLIST = new Set(["CS", "X", "XX"]);
CharacterBuilder._rowSortOrder = 0;
